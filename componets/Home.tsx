...
const Home = () => {
  ...
  const [typeingLoader, setTypeingLoader] = useState(false);
  ...
  return (
    <div className="flex h-dvh overflow-x-hidden">
      ...
      {/* Main Content */}
      <div
        className={`h-dvh w-full flex flex-col justify-center item-center bg-background transition-all duration-300 ${isSidebarOpen ? "md:ml-64" : ""
          }`}
      >
        {messages.length === 0 ? (
          <div className="py-4 w-full mx-auto flex flex-col justify-center items-center gap-2 text-center opacity-0 animate-[fadeIn_2s_ease-in-out_forwards]">
            ...
            <h2 className="font-semibold text-3xl">
              Chat<span className="text-primary">Nova</span> is ready
            </h2>
            <h3 className="font-semibold text-2xl ">
              <span className="text-primary">
                {user?.fullName ? user.fullName.split(" ")[0] : "Guest"}
              </span>
              , how can I help <span className="text-primary">today</span>?
            </h3>
            ...
            <div className="flex gap-2 mt-2 flex-wrap justify-center">
              ...
              >
                <NotebookPen className="text-[#ff7300] mr-2" />{" "}
                Summarize text
              </div>
              <div
                className="text-foreground/50 border-[2px] border-muted rounded-full px-3 py-1 flex justify-center items-center cursor-pointer"
              ...
              >
                <Brain className="text-[yellow] mr-2" />{" "}
                Brainstorm ideas
              </div>
              <div
                className="text-foreground/50 border-[2px] border-muted rounded-full px-3 py-1 flex justify-center items-center cursor-pointer"
              ...
              >
                <Flower className="text-[green] mr-2" />{" "}
                Surprise me
              </div>
              <div
                className="text-foreground/50 border-[2px] border-muted rounded-full px-3 py-1 flex justify-center items-center cursor-pointer"
              ...
              >
                <GraduationCap className="text-[blue] mr-2" />{" "}
                Get advice
              </div>
            </div>
          </div>
                          ...
                          return !inline && match ? (
                            <div className="relative group/code">
                              <button
                                ...
                                className="absolute top-2 right-2 transition-opacity p-1 rounded-md bg-background hover:bg-zinc-700"
                              >
