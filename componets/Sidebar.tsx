...
import { Plus, History, Settings, X, MessageSquare } from 'lucide-react'
  ...
  return (
    ...
    `}>
      <div className="w-full">
        ...
        <button className="w-full bg-[#1e1e1e] hover:bg-[#222] text-white rounded-lg flex items-center justify-start gap-2 p-2 mb-4 transition">
          <Plus size={18} />
          <span>New Chat</span>
        </button>
        ...
        <div className="flex flex-col gap-1 overflow-y-auto max-h-[65vh] scrollbar-thin scrollbar-thumb-[#333] scrollbar-track-transparent">
          {['AI Assistant', 'Marketing Plan', 'Code Debug', 'Daily Ideas'].map((item, i) => (
            <button
              key={i}
      ...
      <div className="flex flex-col gap-2 border-t border-[#222] pt-3">
        <button className="flex items-center gap-2 p-2 px-3 rounded-lg hover:bg-[#222] text-gray-300 transition">
          <History size={16} />
          <span>History</span>
        </button>
        <button className="flex items-center gap-2 p-2 px-3 rounded-lg hover:bg-[#222] text-gray-300 transition">
